...
export class HttpResponse<Data = unknown> {
  ...
  static transformErrorToHttpResponse(err: any) {
    ...
    if (err instanceof HttpResponseException) {
      const option = err.getResponse() as HttpResponseOption;
      return new HttpResponse(option);
    }
    ...
    if (err?.name === 'HttpResponseException') {
      return new HttpResponse(err.response as object);
    }
...
export class HttpResponseException extends HttpException {
  constructor(option: Partial<HttpResponseOption>) {
    const httpResponse = new HttpResponse({
      statusCode: option.statusCode ?? BusinessStatus.INTERNAL_SERVER_ERROR,
      httpStatus: option.httpStatus ?? HttpStatus.INTERNAL_SERVER_ERROR,
      message: option.message ?? 'Internal server error',
      ...option,
    });
    super(httpResponse.getOption(), httpResponse.getHttpStatus());
  }
}
