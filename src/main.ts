...
config({
  path: process.env.APP_ENV_FILE_PATH || undefined,
});
...
async function bootstrap() {
  const app = await NestFactory.create<NestFastifyApplication>(
    AppModule,
    new FastifyAdapter({
      logger: process.env.APP_DEBUG === 'true',
  ...
  await app.listen(process.env.APP_PORT ? Number(process.env.APP_PORT) : 8080, '0.0.0.0');
}
